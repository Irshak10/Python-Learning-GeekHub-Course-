	...
	while choose_number != '4':
		if (choose_number == '1'):
			...
			if user_id == '11':
				...
				with open(f'{user_id}_transactions.data.csv', 'a', newline = '') as f:
					writer = csv.writer(f)
					writer.writerow([f'User added {inc_sum * n_sum} UAH - Amount {n_sum} banknotes on {inc_sum} UAH | ',f])
				print(f'Баланс Банкомату поповнено на {inc_sum * n_sum} UAH\nІнкасацією внеcено {n_sum} банкнот по {inc_sum} UAH\n')
				prestart_menu()
				choose_number = str(start())    
			else:
				# Моя реалізація під-задачі №2 (рандом 10%), при чому при кожному запиті в кожного юзера є змога вловить цю "удачу"					
				lucky_number = random.randint(1, 10)
				lucky_sum = 100
				random_possible = True
				if lucky_number == int(user_id):
					plus_sum = int(input('Введіть суму поповнення: '))
					print(f'Удача на Вашій стороні, Вам на рахунок додатково надається + {lucky_sum} UAH\n')
					user_balance = int(user_balance) + plus_sum + lucky_sum
					with open(f'{user_id}_transactions.data.csv', 'a', newline = '') as f:
						writer = csv.writer(f)
						writer.writerow([f'User added {plus_sum} + {lucky_sum} UAH | '])
					random_possible = False
					print(f'Ваш баланс поповнено на {plus_sum} UAH + {lucky_sum} UAH\n')  
				else:
					plus_sum = int(input('Введіть суму поповнення: '))
					user_balance = int(user_balance) + plus_sum
					with open(f'{user_id}_transactions.data.csv', 'a', newline = '') as f:
						writer = csv.writer(f)
						writer.writerow([f'User added {plus_sum} UAH | '])
				with open(f'balance{user_id}.json', 'w') as f:
					json.dump(user_balance,f)
				if random_possible:	
			...
			if possible:				
				...
				with open(f'{user_id}_transactions.data.csv', 'a', newline = '') as f:
					writer = csv.writer(f)
					writer.writerow([f'User removed {minus_sum} UAH | '])
