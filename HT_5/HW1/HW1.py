...
# Валідація користувача по логіну та паролю (зчитування данних з json файлу) і присвоєння користуванню відповідного id.
def validat():
    ...
    for i in data['users']:
        if login == i['firstName'] and password == i['password']:
        	...
        	print('Логін та пароль вказані не вірно')
    ...
    while choose_number != '4':
        if (choose_number == '1'):
            ...
            with open(f'{user_id}_transactions.data.json', 'a') as f:
                transactions = json.dump(f'User added {plus_sum} UAH \n',f)
            ...
            if minus_sum > user_balance:
                ...
                with open(f'balance{user_id}.json', 'w') as f:
                    user_balance = json.dump(user_balance,f)
            with open(f'{user_id}_transactions.data.json', 'a') as f:
                transactions = json.dump(f'User removed {minus_sum} UAH \n',f)
